					...
					handler: function(_htm) {
						...
						return self.setHTM(_htm, {
							...
							apis: {
								...
								"issues-by-tag": {
									"url": self.config.serviceBaseUri + "/io.devcomp.portal/2-views/issues.json?tag=" + encodeURIComponent(JSON.stringify({name:"fixed",value:"firebug-3.0.0-alpha.3"})),
									"$label": self.config.serviceBaseUri + "/io.devcomp.portal/2-views/issues.json?tag=' + encodeURIComponent(JSON.stringify({name:\"fixed\",value:\"firebug-3.0.0-alpha.3\"}))"
								}
							}
						}).then(function(tag) {
